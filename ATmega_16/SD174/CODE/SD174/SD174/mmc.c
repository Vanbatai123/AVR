...
#define SELECT()	PORTB &= ~0x04	/* MMC CS = L */
#define	DESELECT()	PORTB |=  0x04	/* MMC CS = H */
#define	MMC_SEL		!(PORTB &  0x04)	/* MMC CS status (true:selected) */
#define	INIT_SPI()	spi_init()	/* Initialize SPI port */
...
DSTATUS disk_initialize(void)
{
	...
	if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2 */
	    for (n = 0; n < 4; n++)
		ocr[n] = spi_rx();	/* Get trailing return value of R7 resp */
