 ...
 * myLCD_TFT.h
 *
 * Created: 01/12/2016 19:22:19 
 * Author: Van_BasTai
 */
 
#include "F:\AVR\Header\TFT_Font.c"
...
#define LCD_RESET			4		//RESET
#define LCD_CS				3		//Chip Select
#define LCD_CD				2		//CMD, DATA
#define LCD_WR				1		//WRITE
#define LCD_RD				0		//READ
...
//-----------------------------------------------------------------------------------------------------------
void TFT_Number_thapphan(unsigned int x, unsigned int y,float number,unsigned char size, unsigned int color)
{
	int q;
	q=(number*10)/10;
	TFT_Number_Dec(x, y, q, size, color);
	TFT_PutChar(x + 13, y, '.', size, color);
	q=(int)(number*10);
	q=q%10;
	TFT_Number_Dec(x + 26, y, q, size, color);
}