...
#define CR 0x0D         //Carriage Return Character, '\r' found not working
...
__C_task void main(void)
{
    ...
    unsigned char val;
    SETBIT(DDRB, PB0);    //Data direction register for LED
//    SETBIT(PORTC, PC0);   //Switch OFF LED, default ON, Set to OFF
    ...
    {
        SETBIT(PORTB, PB0);   //Switch LED0 attached to PORTC high, STK 500 Clear to ON
        /* Main loop */
        for(;;)
        {
            ...
            {
                sendchar(CR); // Nothing special to do, just answer OK.
            }
...
unsigned char BlockLoad(unsigned int size, unsigned char mem, long *address)
{
    ...
    if(mem=='E')
    {
        ...
        return CR; // Report programming OK
    }
